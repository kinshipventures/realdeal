...
import { getContacts, addRecordToProject } from '../../lib/airtable'
...
export function AddToProjectModal({ open, onClose, projectId, existingIds, onAdded }: Props) {
  ...
  async function handleAdd() {
    ...
    setSubmitting(true)
    try {
      for (const id of Array.from(selected)) await addRecordToProject(projectId, id)
      onAdded()
      onClose()
    } finally {
      setSubmitting(false)
  ...
  return (
    ...
    >
      <div
        style={{
          width: 440, maxHeight: '70vh', borderRadius: 16,
          background: 'var(--surface-panel)', backdropFilter: 'blur(32px)',
          WebkitBackdropFilter: 'blur(32px)', boxShadow: '0 16px 48px rgba(0,0,0,0.12)',
      ...
      >
        ...
        <input
          ...
          style={{
            ...
            border: '1px solid var(--edge)', background: 'var(--color-bg)',
        ...
        <div style={{ flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 2, minHeight: 120, maxHeight: 320 }}>
          {filtered.length === 0 ? (
            <p style={{ fontSize: 13, color: 'var(--color-text-secondary)', textAlign: 'center', padding: '24px 0' }}>
              {query ? 'No results' : 'All people already added'}
            </p>
            ...
            <label
              ...
              style={{
                ...
                background: selected.has(contact.id) ? 'rgba(37,180,57,0.06)' : 'transparent',
                transition: 'background 0.1s',
              }}
            >
              <input
                type="checkbox" checked={selected.has(contact.id)}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 4 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" onClick={handleClose} style={{
              padding: '8px 16px', borderRadius: 10, border: '1px solid var(--edge)',
              background: 'transparent', fontSize: 13, fontWeight: 500,
              color: 'var(--color-text-secondary)', cursor: 'pointer', fontFamily: 'inherit',
            }}>Cancel</button>
            <button type="button" disabled={selected.size === 0 || submitting} onClick={handleAdd} style={{
              ...
              fontFamily: 'inherit',
            }}>{submitting ? 'Adding...' : 'Add Selected'}</button>
          </div>
        </div>
      </div>
    </div>
