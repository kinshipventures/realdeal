...
BEGIN
  ...
  SELECT incoming.contact
  INTO scoped_contact
  FROM public.get_shared_contacts_with_me() AS incoming
  WHERE incoming.grant_id = target_grant.id
    AND incoming.contact ->> 'id' = updated_contact.id::text
  LIMIT 1;

  IF scoped_contact IS NULL THEN
    RAISE EXCEPTION 'Updated shared contact could not be reloaded';
  END IF;
